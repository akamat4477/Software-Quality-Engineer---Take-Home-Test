...
public class AmazonPage {
	...
	public  void verifySearchBox() throws Exception{
		if(!searchTextBox.isDisplayed()){
			...
			Assert.fail("Search box not found on Amazon app");
		}
	}
	...
	public void verifyPrimeSearchResults(String result) {
	    ...
	    for (int i = 0; i < results.size(); i++) {
	        Assert.assertTrue(results.get(i).getText().contains(result), "Search result validation failed at instance [ + i + ].");
	    }
	}
	...
	public void clickOnAmazonChoice() throws Exception {
		if(searchTextBox.isDisplayed()){
			...
			Assert.fail("Amazon choice option doesn't displayed");
		}
	}
	...
	public void loginAmazonAccount(String userId, String password) {
		...
		{
			System.out.println("Amazon account is already logged in");
		}
	}
